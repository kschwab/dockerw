...
def dockerw_run(args: list) -> None:
    ...
    parser.add_argument('--login-message', dest='dockerw_login_message', default=None,
                        help='Login message to display. Default is nothing (venv must be enabled)')
    ...
    if '--env=DOCKERW_VENV=1' in args:
        ...
        if not [arg for arg in args if arg.startswith('--env=DOCKERW_LOGIN_MESSAGE')]:
            args.append(f'--env=DOCKERW_LOGIN_MESSAGE={post_args.get("dockerw_login_message","")}')
        print(f'mkdir -p {DOCKERW_VENV_HOME_PATH}',
              f'if [ -z "$SHELL" ]; then SHELL=sh; fi',
              ...
              f'echo \'    echo -e "$DOCKERW_LOGIN_MESSAGE"\' >> {DOCKERW_VENV_RC_PATH}',
              ...
              f'echo \'export PS1="\\e[7m📦{prompt_banner}\\e[0m\\n[\\u@\\h \\W]\\$ "\' >> {DOCKERW_VENV_RC_PATH}',
              f'echo \'if [ "$(id -u)" != "{DOCKERW_UID}" ] && [ "$SUDO_UID" != "{DOCKERW_UID}" ]; then\' >> {DOCKERW_VENV_RC_PATH}',
              f'echo "  cd $PWD" >> {DOCKERW_VENV_RC_PATH}',
              f'echo "  export HOME=/home/{DOCKERW_UNAME}" >> {DOCKERW_VENV_RC_PATH}',
              ...
              f'run_user_cmd() {{',
              f'  local _IS_EXEC=$1; shift',
              f'  local _USERSPEC=$1; shift',
              f'  local _USERNAME=$1; shift',
              f'  if $_IS_EXEC; then local _EXEC=exec; fi',
...
def _dockerw_login_message_args(parsed_args: dict, parsed_image_cmd: list, post_args: dict) -> list:
    post_args['dockerw_login_message'] = parsed_args['dockerw_login_message']
    return []
